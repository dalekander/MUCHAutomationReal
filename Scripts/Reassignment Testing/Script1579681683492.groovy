...
'set different user into visitor filter input'
WebUI.setText(findTestObject('Homepage/Page_Much application/much_ui_homepage_visitor_input'), GlobalVariable.reassignment_user)

'click on search button'
WebUI.click(findTestObject('Assignment Management Page/Page_Much application/much_ui_assignmentpage_search_button'))

'extract the number of filtered visit case and store it in reassignedBefore variable'
//String reassignedBefore = WebUI.getText(findTestObject('Object Repository/Assignment Management Page/Page_Much application/Filtered Button')).replaceAll("[^0-9.]", "")
String reassignedBefore = WebUI.getText(findTestObject('Object Repository/Assignment Management Page/Page_Much application/much_ui_assignmentpage_filtered_number')).replaceAll('[^0-9.]', '')
println ('reassigned before: ' + reassignedBefore)
...
WebUI.click(findTestObject('Homepage/Page_Much application/much_ui_homepage_visitcasestate_filter_Assigned'))
...
GlobalVariable.selectedVisitCaseID_RA = rows_table1.get(0).findElements(By.tagName('td')).get(14).getText()
GlobalVariable.selectedContracNo_RA = rows_table1.get(0).findElements(By.tagName('td')).get(2).getText()
GlobalVariable.selectedCreationDate_RA = rows_table1.get(0).findElements(By.tagName('td')).get(8).getText()
...
if (found == true) {
	...
	'now we check the number of visit case assigned after reassignment'
	...
	'now check the number of visit case assigned to previous user'
	'set the reassignment user into visitor filter'
	WebUI.setText(findTestObject('Homepage/Page_Much application/much_ui_homepage_visitor_input'), GlobalVariable.reassignment_user)
	
	'click on search button'
	WebUI.click(findTestObject('Assignment Management Page/Page_Much application/much_ui_assignmentpage_search_button'))
	
	'extract the text inside filtered button and put it in reassginedAfter variable'
	//String reassignedAfter = WebUI.getText(findTestObject('Object Repository/Assignment Management Page/Page_Much application/Filtered Button')).replaceAll("[^0-9.]", "")
	String reassignedAfter = WebUI.getText(findTestObject('Object Repository/Assignment Management Page/Page_Much application/much_ui_assignmentpage_filtered_number')).replaceAll('[^0-9.]', '')
	println ('reassigned after: ' + reassignedAfter)
	
	'calculate the difference between reassignedBefore and reassignedAfter'
	int reassignedDif = 0
	reassignedDif = Integer.parseInt(reassignedAfter) - Integer.parseInt(reassignedBefore)
	
	'if the differece is decrease by 1 then it is the expected result, if not then it is not'
	if( reassignedDif  == -1)
	{
		println('TESTING SUCCESFULL -  Number of assigned visit case to ' + GlobalVariable.reassignment_user + ' decreased by ' + reassignedDif)
	}
	else
	{
		println('TESTING UNSUCCESSFULL - Number of assigned visit case to ' + GlobalVariable.reassignment_user + ' is not decreased by 1')
		KeywordUtil.markFailedAndStop('TESTING UNSUCCESSFULL - Number of assigned visit case to ' + GlobalVariable.reassignment_user + ' is not decreased by 1')
	}
}
